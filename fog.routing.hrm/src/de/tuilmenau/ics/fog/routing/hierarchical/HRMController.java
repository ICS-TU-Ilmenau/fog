								...
								if(tCoordinator != null){
									if(tCoordinator.isThisEntityValid()){
										if((HRMConfig.Measurement.AUTO_SKIP_COORDINATOR_PROXY_INVALIDATION) &&
											 ...
											 */
											if(getSimulationTime() < tProxy.getTimeout() + HRMConfig.Hierarchy.COORDINATOR_ANNOUNCEMENTS_INTERVAL_LT_EXISTENCE_TIME / 2){
...
//						}
	...
	private static String getDataRateStr(Integer pBytes, double pPeriod)
	{
		...
		if((pBytes != null) && (pPeriod > 0)){
			double tDataRate = ((double)pBytes / pPeriod);
			DecimalFormat tFormat = new DecimalFormat("0.0#");
			tResult = tFormat.format(tDataRate);
		}
	...
	{
		//TODO for network policies: check if a neighbor wants to explore its neighbor -> select if we want to join its cluster or not
		if (HRMConfig.DebugOutput.GUI_SHOW_TOPOLOGY_DETECTION){
...
//				}
	...
	public long getMaxDataRate(HRMID pDestination)
	{
		...
		RoutingEntry tRoutingEntry = getHRS().getRoutingDecision(pDestination, RoutingEntry.INFINITE_DATARATE, 0, null, null);
	...
	public long getMinDelay(HRMID pDestination)
	{
		...
		RoutingEntry tRoutingEntry = getHRS().getRoutingDecision(pDestination, 0, -1, null, null);
		if(tRoutingEntry != null){
